...
class LaCTBlock(nn.Module):
    ...
    def __init__(self, config: LaCTSWIGLUConfig, layer_idx: int):
        ...
        self.attn = LaCTSWIGLULayer(
            ...
            # === CSDM (fine-grained) passthrough ===
            num_slices=config.num_slices,     # E>=2 enables CSDM
            router_act=config.router_act,       # "silu"|"relu"|"softplus"|"square"|"identity"
            router_norm=config.router_norm,     # "l1"|"l2"|"none"
        )
    ...
    def _init_weights(
        ...
        if isinstance(module, LaCTSWIGLULayer):
            ...
            if module.num_slices >= 1:
                nn.init.ones_(module.router)
                # Per-token, per-head non-competitive router: outputs n_h * E activations
                # Precompute expert channel slices over dh (even split; distribute remainder to early slices)
                d_h_full = int(module.d_h)
                base = d_h_full // module.num_slices
                rem  = d_h_full %  module.num_slices
                starts, sizes = [], []
                s = 0
                for e in range(module.num_slices):
                    sz = base + (1 if e < rem else 0)
                    starts.append(s); sizes.append(sz); s += sz
                module.register_buffer("_csdm_slice_starts", torch.tensor(starts, dtype=torch.int32), persistent=False)
                module.register_buffer("_csdm_slice_sizes",  torch.tensor(sizes,  dtype=torch.int32), persistent=False)
